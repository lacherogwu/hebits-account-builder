...
// startup banner reporting a version that is not the one actually installed, on a service
// whose only build-identity signal those two are.
//
...
// where it runs this is dist/server.mjs and nothing else - no package.json, no node_modules -
// so a build that stopped inlining would not report a wrong version, it would fail to start at
// all (ERR_MODULE_NOT_FOUND), restarting forever under a supervisor that restarts on exit.
// test/bundle.test.ts pins that against the built
